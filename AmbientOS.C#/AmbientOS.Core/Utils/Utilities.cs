...
namespace AmbientOS.Utils
{
    public static class Utilities
    {
        ...
        /// <summary>
        /// Creates a string table, of which each column has a fixed width (e.g. all strings of that column are padded to the same width)
        /// </summary>
        public static string[,] CreateStringTable<T>(T[] rows, params Func<T, string>[] columns)
        {
            var cells = new string[rows.Count(), columns.Count()];
            var widths = new int[columns.Count()];

            // pass 1: fill cells and calculate column widths
            for (int i = 0; i < rows.Count(); i++)
                for (int j = 0; j < columns.Count(); j++)
                    widths[j] = Math.Max(widths[j], (cells[i, j] = columns[j](rows[i])).Count());

            // pass 2: pad cells to calculated column width
            for (int i = 0; i < rows.Count(); i++)
                for (int j = 0; j < columns.Count(); j++)
                    cells[i, j] = cells[i, j] + new string(' ', widths[j] - cells[i, j].Length);
        ...
        {
            return Enum.GetName(value.GetType(), value).CamelCaseToNormal();
        }
